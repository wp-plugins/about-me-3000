...
function control_aboutme() {
    ...
    <?php 
        for ($i=0;$i<count($arr_am_titles);$i++) {
              ...
              ?>
              <tr>
              <th><label for="aboutme-<?=$tag_id?>"><?=$arr_am_titles[$i]?> profile ID:</label></th>  
              <td><input type="text" id="aboutme-<?=$tag_id?>" name="aboutme-<?=$tag_id?>" value="<?php echo $options[$tag_id];?>" /> <img src="<?=get_bloginfo('wpurl')?>/wp-content/plugins/about-me-3000/<?=$tag_id?>.png" border="0"> 
                  <?
                  if($i == 0) {?>
                      <? if($options['vanityurl_on'])   $url = "http://www.facebook.com/"; 
                                                   else $url = $arr_am_urls[$i];                        
                      ?>
                      <small>(<?=$url?>YourID)</small><br />
                  ...
                  <?} else { ?>
                       <small>(<?=$arr_am_urls[$i]?>YourID)</small><br />
                  <? } ?>
                  <input class="checkbox" type="checkbox" id="aboutme-<?=$tag_id?>_on" name="aboutme-<?=$tag_id?>_on" value="1" <?php echo (($options[$tag_id."_on"]=='1')?' checked=1':''); ?>/>
...
function control_aboutme_() {
   ?>
   <p>  
     To configure this widget go to (Settings/About Me 3000) or click <a href='<?=get_bloginfo('wpurl')?>/wp-admin/options-general.php?page=About%20Me%203000'>here</a>
   </p>
   <?  
}

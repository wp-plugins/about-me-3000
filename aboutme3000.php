...
function widget_aboutme($args) {
    ...
    for ($i=0;$i<count($arr_am_titles);$i++) {
        ...
        if($options[$tag_id.'_on']) echo "<a style='padding:2px' href='".$arr_am_urls[$i]."".$options[$tag_id]."'><img src='".get_bloginfo('url')."/wp-content/plugins/aboutme3000/".$tag_id.".png' border='0'></a>";
    }
...
<!--
    ...
    document.write("<img src='<?=get_bloginfo('url')?>/wp-content/plugins/aboutme3000/email.png' border='0'>");
    document.write("</a>");
//-->
</script>
        <?
        //echo "<a style='padding:2px' href='mailto:".$options['email']."'><img src='".get_bloginfo('url')."/wp-content/plugins/aboutme3000/email.png' border='0'></a>";
    }    
    echo "</div>";
